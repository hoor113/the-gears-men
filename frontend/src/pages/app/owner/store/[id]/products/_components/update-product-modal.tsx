...
const schema: yup.ObjectSchema<ProductFormValues> = yup.object().shape({
  storeId: yup.string().required('Store ID là bắt buộc'),
...
const UpdateProductModal = NiceModal.create((props: TUpdateStoreModalProps) => {
  const queryClient = useQueryClient();
  const modal = useModal();
  const { storeId, storeName, row, selectedRange } = props;

  const { mutate } = useMutation({
    mutationFn: (data: any) => ownerProductsService.update(data),
    onSuccess: () => {
  ...
  const { handleSubmit, control, formState } = useForm<ProductFormValues>({
    defaultValues: {
      storeId: storeId || '',
      storeName: storeName || '',
      name: row?.name || '',
  ...
  return (
    <Dialog {...muiDialogV5(modal)} fullWidth maxWidth="md" scroll="paper">
