...
import { Delete as DeleteIcon, Edit as EditIcon, LocalOffer as VoucherIcon } from '@mui/icons-material';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { enqueueSnackbar } from 'notistack';
import NiceModal from '@ebay/nice-modal-react';
import ConfirmModal from '@/components/confirm-modal';
import ownerVoucherService from '../_services/voucher.service';
import { EDiscountCalculationMethod, EDiscountCodeType } from '../_services/voucher.model';
...
const OwnerVoucherItem: React.FC<TProps> = ({ voucher }) => {
    ...
    const renderTitle = () => {
        const discountText =
            voucher?.discountCalculationMethod === EDiscountCalculationMethod.Percentage
                ? `${voucher?.discountQuantity}%`
                : `${voucher?.discountQuantity?.toLocaleString()}₫`;

        const methodText =
            voucher?.type === EDiscountCodeType.ShippingDiscount ? 'giá ship' : 'giá sản phẩm';
    ...
    const stockText = voucher?.quantity > 0 ? `Còn lại ${voucher?.quantity}` : 'Đã hết';
                ...
                {!isMobile && (
                    <Box className="absolute top-2 right-2 flex gap-1 bg-white/90 p-1 rounded-full shadow-sm">
                        <Tooltip title="Sửa voucher">
                            <IconButton size="small"  onClick={() => NiceModal.show(EditDiscountCodeModal, {
                                row: voucher
                            })}>
            ...
            <Box className="p-3 space-y-1">
                ...
                {isMobile && (
                    <Box className="flex justify-center gap-2 mt-3">
                        <Tooltip title="Sửa voucher">
                            <IconButton size="medium" onClick={() => NiceModal.show(EditDiscountCodeModal, {
                                row: voucher
                            })}>
