...
const SaleProductPage = () => {
    ...
    const [selectedRange, setSelectedRange] = useState({ label: "Tất cả", min: 0, max: Infinity });
    ...
    const [dynamicPriceRanges, setDynamicPriceRanges] = useState<any[]>(defaultPriceRanges);
    ...
    const { data: getAllCategoryProduct = [], refetch, isLoading } = useQuery<any>({
        queryKey: ["products/getAll", selectedRange],
        queryFn: () =>
            productsService.getDailyDiscount<any>('/GetDailyDiscount', {
                skipCount: 0,
                maxResultCount: 100000,
                minPrice: selectedRange.min !== 0 ? selectedRange.min : undefined,
                maxPrice: selectedRange.max !== Infinity ? selectedRange.max : undefined,
            }),
    });
    ...
    useEffect(() => {
        if (getAllCategoryProduct?.length && !hasInitializedPriceRange.current) {
            const prices = getAllCategoryProduct.map((p: any) => p?.priceAfterDiscount ?? p?.price);
            ...
            setDynamicPriceRanges([{ label: "Tất cả", min: 0, max: Infinity }, ...ranges]);
    ...
    const sortedProducts = useMemo(() => {
        return [...getAllCategoryProduct].sort((a: any, b: any) => {
            ...
            switch (sortOrder) {
                ...
                case "discountDesc":
                    const discountA = ((a.price - a.priceAfterDiscount) / a.price) || 0;
                    const discountB = ((b.price - b.priceAfterDiscount) / b.price) || 0;
    ...
    }, [getAllCategoryProduct, sortOrder]);
    ...
    const priceFilterComponent = (
        <FormControl component="fieldset">
            <RadioGroup
                ...
                onChange={(e) => {
                    const selected = dynamicPriceRanges.find(p => p.label === e.target.value);
    ...
    return (
        <Box className="bg-gray-100 min-h-screen p-4">
            ...
            <Paper elevation={3} className="p-4 grid grid-cols-12 gap-4">
                ...
                {!isSmallScreen && (
                    <Divider orientation="vertical" flexItem className="col-span-1 border-r-2 border-gray-300" />
                )}

                {/* Main content */}
                <Box className={`space-y-4 ${isSmallScreen ? "col-span-12" : "col-span-9"}`}>
                    {/* Filter + sort buttons */}
