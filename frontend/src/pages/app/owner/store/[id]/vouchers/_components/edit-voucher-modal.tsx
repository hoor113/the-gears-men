...
type TEditDiscountCodeModalProps = {
    filter?: any;
    row?: any;
};

const EditDiscountCodeModal = NiceModal.create((props: TEditDiscountCodeModalProps) => {
    ...
    const fields = useMemo<TCrudFormField[]>(() => [
        ...
        },
        {
            ...
            required: true,
            colSpan: 6,
            options: Object.entries(DiscountCalculationMethodLabels).map(([value, label]) => ({
                value,
                label,
            })),
        ...
        },
        {
            ...
            required: true,
            colSpan: 6,
            defaultValue: row?.expiryDate || new Date().toISOString().slice(0, 10), // Mặc định ngày hiện tại
        },
        {
            name: 'quantity',
            label: 'Số lượng mã',
    ...
    const schema = useMemo(() =>
        yup.object().shape({
            ...
            type: yup.string().oneOf(Object.values(EDiscountCodeType)).required('Loại mã giảm giá là bắt buộc'),
