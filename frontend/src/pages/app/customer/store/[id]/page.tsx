...
const SingleStorePage = () => {
    ...
    const { data: getAllStoreProduct = [], refetch, isLoading } = useQuery({
        queryKey: ["products/getAll", id, selectedRange],
        queryFn: () =>
    ...
    useEffect(() => {
        if (getAllStoreProduct?.length && !hasInitializedPriceRange.current) {
            const prices = getAllStoreProduct.map((p: any) => p?.priceAfterDiscount ?? p?.price);
