    ...
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!allowedRoles.includes(decoded.role)) {
                return res.status(403).json(BaseResponse.error("Access denied", 403));
            }
            next();
        } catch (error) {
...
export const isSelfOrAuthorizedRoles = (allowedRoles: string[]) => {
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            const requestUserId = req.query?.id || req.body?.id || req.params?.id;
            if (!requestUserId) {
                return res.status(400).json(BaseResponse.error("Missing id parameter in request", 400));
            }
